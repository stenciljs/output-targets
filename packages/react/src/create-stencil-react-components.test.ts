...
describe('createStencilReactComponents', () => {
  it('should generate a basic react component without events', () => {
    ...
    expect(result).toContain(`import { createCustomElementReactComponent } from '@stencil/react-output-target/runtime';`);
    expect(result).toContain(`import type { EventName, Options } from '@stencil/react-output-target/runtime';`);
  ...
  it('should organize and format imports correctly', () => {
    ...
    const createComponentImportIndex = lines.findIndex((line) => line.includes(`import { createCustomElementReactComponent }`));
