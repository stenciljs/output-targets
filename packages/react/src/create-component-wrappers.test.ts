...
describe('createComponentWrappers', () => {
  ...
  it('should generate a react component wrapper with ES modules', async () => {
    ...
    expect(componentsSourceFile.getFullText()).toMatchSnapshot('barrel file');

    const sourceFile = sourceFiles[1];

    expect(sourceFile.getFullText()).toMatchSnapshot('component file');
  });
  ...
  it('should generate a server barrel when esModules: true and hydrateModule are both set', async () => {
    ...
    expect(serverBarrelText).toMatchSnapshot('server barrel');
