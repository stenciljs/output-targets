...
`);
  ...
  it('should transform tag names when tagNameTransform is provided', async () => {
    const project = new Project({ useInMemoryFileSystem: true });
    const sourceFiles = await createComponentWrappers({
      components: [
        {
...
`);
  ...
  it('should transform tag names in server side rendering components', async () => {
    const project = new Project({ useInMemoryFileSystem: true });
    const sourceFiles = await createComponentWrappers({
      components: [
        {
    ...
    expect(sourceFiles.length).toBe(2);
    const serverFile = sourceFiles[1];

    const code = serverFile.getFullText();
    expect(code).toContain(`tagName: 'my-component-v2',`);
    expect(code).toContain(`properties: { hasMaxLength: 'max-length' },`);
  });
});
